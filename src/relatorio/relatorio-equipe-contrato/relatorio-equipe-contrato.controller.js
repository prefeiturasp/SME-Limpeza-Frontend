(() => {
  ...
  angular
    .module('relatorio.relatorio-equipe-contrato')
    .controller('RelatorioEquipeContratoController', RelatorioEquipeContratoController);

  RelatorioEquipeContratoController.$inject = ['$rootScope', '$location', 'controller', 'RelatorioEquipeContratoRest', 'tabela', 'UnidadeEscolarUtils',
    'PrestadorServicoUtils', 'ContratoUtils'];

  function RelatorioEquipeContratoController($rootScope, $location, controller, dataservice, tabela, UnidadeEscolarUtils,
    PrestadorServicoUtils, ContratoUtils) {
        ...
        const colunas = [
          ...
          },
          {
            data: '', title: 'Contrato', renderWith: (v1, v2, data) =>
              `
							<h5 style="font-weight: 100">${data.codigo}</h5>
							<small>${data.descricao}</small>
						`
          },
          { data: 'prestadorServico', title: 'Prestador de Serviço', renderWith: tabela.formatarPrestadorServico },
          {
            data: 'quantidadeUnidadesEscolar', title: 'UE\'s', cssClass: 'text-right', width: 5, renderWith: (total) =>
              `<div class="badge badge-pill badge-primary">${total}</div>`
          },
          {
    ...
    function exportar() {
      ...
      function success(response) {
        ...
        a.href = fileUrl;
        a.download = `relatorio-multa-equipe-contrato-${moment().format('DDMMyyyyHHmmss')}.csv`;
        a.click();
      }
