(function () {
  'use strict';

  angular.module('qrScanner', ["ng"]).directive('qrScanner', ['$interval', '$window', function ($interval, $window) {
    return {
      restrict: 'E',
      scope: {
        ngSuccess: '&ngSuccess',
        ngError: '&ngError',
        ngVideoError: '&ngVideoError'
      },
      link: async function (scope, element, attrs) {

        window.URL = window.URL || window.webkitURL || window.mozURL || window.msURL;
        navigator.getUserMedia = navigator.getUserMedia || navigator.mediaDevices.getUserMedia || navigator.webkitGetUserMedia || navigator.mozGetUserMedia || navigator.msGetUserMedia;

        var height = attrs.height || 300;
        var width = attrs.width || 250;

        var cheight = height < 500 ? 500 : height;
        var cwidth = width < 500 ? 500 : width;

        var video = $window.document.createElement('video');

        video.setAttribute('autoplay', '');
        video.setAttribute('width', '100%');
        video.setAttribute('height', '');
        video.setAttribute('muted', ''); // Fix for iOS Safari from https://leemartin.dev/hello-webrtc-on-safari-11-e8bcb5335295
        video.setAttribute('playsinline', '') // Fix for iOS Safari from https://leemartin.dev/hello-webrtc-on-safari-11-e8bcb5335295
        // video.setAttribute('style', '-moz-transform:rotateY(-180deg);-webkit-transform:rotateY(-180deg);transform:rotateY(-180deg);');
        var canvas = $window.document.createElement('canvas');
        canvas.setAttribute('id', 'qr-canvas');
        canvas.setAttribute('width', cwidth);
        canvas.setAttribute('height', cheight);
        canvas.setAttribute('style', 'display:none;');

        angular.element(element).append(video);
        angular.element(element).append(canvas);
        var context = canvas.getContext('2d');
        var stopScan;

        var scan = function () {
          if ($window.localMediaStream) {
            context.drawImage(video, 0, 0);
            try {
              qrcode.decode();
            } catch (e) {
              scope.ngError({ error: e });
            }
          }
        }

        var successCallback = function (stream) {

          try {
            video.srcObject = stream;
          } catch (error) {
            video.src = (window.URL && window.URL.createObjectURL(stream)) || stream;
          }

          $window.localMediaStream = stream;
          scope.video = video;
          video.play();
          stopScan = $interval(scan, 600);
        }

        // Call the getUserMedia method with our callback functions
        //if (navigator.getUserMedia) {
        var constraint = {
          audio: false,
          video: {
            facingMode: { ideal: "environment" }
          }
        };

        const stream = await navigator.mediaDevices.getUserMedia(constraint);
        video.srcObject = stream;
        $window.localMediaStream = stream;
        scope.video = video;
        video.play();
        stopScan = $interval(scan, 600);

        // navigator.mediaDevices.getUserMedia(constraint, successCallback, function (e) {
        //     scope.ngVideoError({ error: e });
        // });

        // } else {
        //    scope.ngVideoError({ error: 'Native web camera streaming (getUserMedia) not supported in this browser.' });
        //}

        qrcode.callback = (data) => {
          scope.ngSuccess({ data: data });
        };

        element.on('$destroy', () => {

          if ($window.localMediaStream) {
            $window.localMediaStream.getVideoTracks()[0].stop();
          }

          if (stopScan) {
            $interval.cancel(stopScan);
          }

        });

      }
    }
  }]);
})();
